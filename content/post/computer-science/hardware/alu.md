...
XOR gate（Exclusive-OR gate、异或门）就是实现了异或运算的逻辑电路，异或门由与门、或门、非门组成（见图：**alu.drawio.html 12-2**）。
...
half adder（半加器）由异或门、与门组成（见图：**alu.drawio.html 14-2**）。可以处理两个 1 位二进制数的加法运算，产生 2 个输出：一个 carry（进位）、一个 sum（和）。

| 操作数 1 | 操作数 2 | 运算结果 | carry  | sum |          示意图           |
|:-----:|:-----:|:----:|:---:|:---:|:----------------------:|
...
full adder（全加器）由半加器、或门组成（见图：**alu.drawio.html 16-2**）。可以处理三个 1 位二进制数的加法运算，产生 2 个输出：一个 carry、1 个 sum。

| 操作数 1 | 操作数 2 | 操作数 3 | 运算结果 | carry  |  sum  |           示意图           |
|:-----:|:-----:|:-----:|:----:|:---:|:---:|:-----------------------:|
...
运算的电路由半加器、全加器组成（见图：**alu.drawio.html 18-2**）。可以处理两个 2 位二进制数加法运算，产生 3 个输出：一个 carry、2 个 sum（注意顺序）。

这里简要描述一下加法运算的逻辑，就是电路里的半加器和全加器是怎么工作的。用 0b11 + 0b11 举例。为了描述方便，这里借用 10 进制的个位、十位、百位的概念。先把 0b11 拆成十位的 1 和个位的 1。

在进行加法运算的时候，先进行个位和个位的加法，这里只有两个参数，两个个位，所以由半加器进行运算。1 + 1 = 10，产生 2 个输出：carry = 1、sum1 = 0。然后进行十位和十位的加法，这里有三个参数，两个十位还有个位相加的进位，所以这里需要全加器进行运算。1 + 1 + 1 = 11，产生 2 个输出：carry = 1、sum2 = 1。所以最后的结果就是 0b110，carry = 1、sum2 = 1、sum1 = 0。

| 操作数 A | 操作数 B | 运算结果  |  carry  | sum2 | sum1 |           示意图           |
|:-----:|:-----:|:-----:|:----:|:---:|:---:|:-----------------------:|
...
参考"两个 2 位二进制数加法"的运算电路，就可以设计出"两个 n 位二进制数加法"的运算电路。这里画一个 8 bit ripple carry adder（8 位行波进位加法器）的示意图（见图：**alu.drawio.html 20-2**）。可以处理两个 8 位二进制数的加法运算，产生 9 个输出：一个 carry、8 个 sum（注意顺序）。
...
计算 0b11111111 + 0b11111111。两个数据输入就是 0b11111111 和 0b11111111、操作码输入就是 0b0001（表示加法）。这个时候，因为 0b11111111 + 0b11111111 = 0b111111110，它溢出了。所以数据输出会是 0b11111110、overflow 会是 1。

计算 0b11111111 == 0b11111111。判断两个数字是否相等，计算机是不会直接比较的，可以用相减之后和 0 比较的思路。那么，两个数据输入就是 0b11111111 和 0b11111111、操作码输入就是 0002（表示减法）。这个时候，因为 0b11111111 - 0b11111111 = 0b00000000，为 0。所以数据输出会是 0b00000000、zero 会是 1。

计算 0b11111111 < 0b11111110。判断两个数字的大小，计算机是不会直接比较的，可以用相减之后和 0 比较的思路。那么，两个数据输入就是 0b11111111 和 0b11111110、操作码输入就是 0002（表示减法）。这个时候，因为 0b11111111 - 0b11111110 = 0b00000001，大于 0。所以数据输出会是 0b00000001、negative 会是 0（表示 0b11111111 - 0b11111110 > 0）。如果是 0b11111110 - 0b11111111，那 negative 就会是 1（表示 0b11111110- - 0b11111111 < 0）。
